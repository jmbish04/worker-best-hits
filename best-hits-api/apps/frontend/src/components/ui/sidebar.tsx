...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  // We use openProp and setOpenProp to control it from outside.
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-1 top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 peer-hover/menu-button:opacity-100 peer-data-[state=open]/menu-button:opacity-100 [&>svg]:size-4 [&>svg]:shrink-0",
        // Increases the hit area of the button on mobile.
        "after:absolute after:-inset-2 after:md:hidden",
        ...
        "group-data-[collapsible=icon]:hidden",
        showOnHover &&
          "opacity-0 group-hover/menu-item:opacity-100 peer-focus-visible/menu-button:opacity-100 peer-hover/menu-button:opacity-100",
        className
      )}
      {...props}
    />
  ...
  ({ className, ...props }, ref) => {
    return (
      <ul
        ...
        className={cn("mx-3.5 flex w-full min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2.5 py-0.5", className)}
        {...props}
      />
    )
  }
